...
It appears that most of our variables are correlated, which limits what may include in a regression model. However, what if we want to test how a set of multiple variables correlates with another set of multiple variables? In mathematical terms, we split our data into two (not necessarily equally sized!) matrices: $\textbf{p}_1$ and $\textbf{p}_2$. Although the number of variables in each of the $\textbf{p}_i$ matrices can be unequal, the number of observations must be equal for the analysis to work.  
...
We are not going to concern ourselves with how to calculate eigenvalues, as we can do this easily in R. Essentially each $Y_i$ is an _eigenvalue_ (AKA Principal Component) and each $e_i$ is an _eigenvector_ with p rows corresponding to p variables (essentially they are scale factors). Each $Y_i$ (formally presented as $\lambda_i$) is essentially a projection of all of the variables, scaled by their corresponding eigenvectors in a 1-dimensional subspace spanning the column space of the initial data matrix. This projection is what we see when we visualize a PCA using a biplot.  
...
pd<-read.csv("/Users/alexthompson/Dropbox/PhD data/Stats stuff/Matt Tomaszewski/wrc/masterpd.csv")%>%
  dplyr::mutate(psipd=abs(psipd))%>%
  dplyr::mutate(psimd=abs(psimd))
leaf<-read.csv("/Users/alexthompson/Dropbox/PhD data/Stats stuff/Matt Tomaszewski/wrc/leaftraits.csv")
elements<-read.csv("/Users/alexthompson/Dropbox/PhD data/Stats stuff/Matt Tomaszewski/wrc/CN.csv")

# run the PCA on just the water potential data
pd_pca<-prcomp(pd[3:6],center=TRUE,scale.=TRUE)
pd_summary<-as.data.frame(summary(pd_pca)$importance) #69% of the dat
group<-pd$Group
...
all_summary<-as.data.frame(summary(all_pca)$importance) # 65% with 2; 79% with 3!
group<-all$Microsite
all_plot12<-ggbiplot(all_pca,choices=1:2,ellipse=TRUE,groups=group)
all_plot13<-ggbiplot(all_pca,choices=c(1,3),ellipse=TRUE,groups=group)
all_plot14<-ggbiplot(all_pca,choices=c(1,4),ellipse=TRUE,groups=group)
all_plot15<-ggbiplot(all_pca,choices=c(1,5),ellipse=TRUE,groups=group)
...
allpca_kab<-kbl(all_pca$rotation,align='c',table.attr = "style = \"color: black;\"",caption="Predawn, Leaf, & Element Traits",digits=2)%>%
